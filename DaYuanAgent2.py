...
##                print(data)
                ...
                if( step <= 640):    
                    ms_policy.add_transition(n_s_ms, n_p[0], ms_reward +  gamma * mn_reward , fw_ms[1], done)
                    mn_policy.add_transition(n_s_mn, n_p[1], mn_reward + (gamma * ms_reward + gamma * pg_reward)/2 , fw_mn[1], done)
                    pg_policy.add_transition(n_s_pg, n_p[2], pg_reward + (gamma * mn_reward + gamma * hn_reward)/2 , fw_pg[1], done)
                    hn_policy.add_transition(n_s_hn, n_p[3], hn_reward + (gamma * pg_reward + gamma * dz_reward)/2 , fw_hn[1], done)
                    dz_policy.add_transition(n_s_dz, n_p[4], dz_reward +  gamma * hn_reward, fw_dz[1], done)
    ...
    #            time.sleep(0.2)
        ...
        if(i % 20 == 0):
            ...
            ms_policy.save('E:\\tlc-sim\\model\\model2\\ms_', i)
            mn_policy.save('E:\\tlc-sim\\model\\model2\\mn_', i)
            pg_policy.save('E:\\tlc-sim\\model\\model2\\pg_', i)
            hn_policy.save('E:\\tlc-sim\\model\\model2\\hn_', i)
            dz_policy.save('E:\\tlc-sim\\model\\model2\\dz_', i)
        s.sendall(b'restart\r\n')
